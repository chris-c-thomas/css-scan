...
import {type AppState, type ScanOptions} from './types.js';
import {createReporter, type ReporterFormat} from './reporter/index.js';
import {buildSummary, type ScanSummary} from './reporter/summary.js';
...
export const App: React.FC<AppProps> = ({initialUrl, depth = 0, maxPages = 1, format = 'css'}) => {
	const {exit} = useApp();
		...
		if (state === 'SCANNING') {
			const runScan = async () => {
				try {
					...
					const report = await scanCssCoverage(url, options);

					// Generate output files
					const reporter = createReporter(format);
					const outputFiles = await reporter.generate(report, {outputDir: process.cwd()});

					const summary = buildSummary(report, outputFiles);
					setResult(summary);
					setState('SUCCESS');
				} catch (err: any) {
	...
	if (state === 'SUCCESS' && result) {
		...
		return (
			<Box flexDirection="column" padding={1} borderStyle="single" borderColor="gray">
				...
				<Box marginTop={1}>
					<Box width={20}><Text>Used Rules:</Text></Box>
					<Text bold color="green">{result.usedRules} / {result.totalRules}</Text>
				</Box>

				<Box>
					<Box width={20}><Text>Unused Rules:</Text></Box>
					<Text bold color={statusColor}>{result.unusedRules} / {result.totalRules}</Text>
				</Box>

				<Box marginTop={1} flexDirection="column" borderStyle="round" borderColor="green" paddingX={1}>
					{result.outputFiles.map(file => (
						<Text key={file}>Exported: <Text bold color="blue">{file}</Text></Text>
					))}
				</Box>
			</Box>
		);
